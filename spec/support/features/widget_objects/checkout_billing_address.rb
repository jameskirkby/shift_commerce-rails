module MyApp
  module Test
    module ComponentObject
      class CheckoutBillingAddress < CapybaraObjects::ComponentObject
        FIELDS = [:first_name, :middle_names, :last_name, :address_line_1, :address_line_2, :address_line_3, :city, :state, :postcode]
        ctype "checkout_billing_address"
        locator :css, "[data-behavior='checkout_billing_address']"
        ...
        # Validates that the billing address equals the values given
        # @param [MyApp::Test::Models::Address] address
        def assert_billing_address_equals(address, disabled: false)
          errors = []
          FIELDS.each do |field|
            begin
              root_node.assert_selector(:field, "cart[billing_address_attributes][#{field}]", with: address.send(field), disabled: disabled)
            rescue Capybara::ExpectationNotMet => ex
        ...
        # Selects the given address as the billing address
        # @param [MyApp::Test::Models::Address] address
        ...
        def assert_all_fields_empty
          FIELDS.each do |field|
            root_node.assert_selector(:field, "cart[billing_address_attributes][#{field}]", with: "")
          end
        end

        def assert_all_fields_disabled
          FIELDS.each do |field|
            root_node.assert_selector(:css, "*[name='cart[billing_address_attributes][#{field}]'][disabled]")
          end
        end
        ...
        def select_from_saved_node
          scoped_find(:css, "select[name='cart[billing_address_id]']")
        end
        ...
        def node_for_field(field)
          scoped_find(:css, "*[name='cart[billing_address_attributes][#{field}]']")
        end

        def allow_edit_node
          scoped_find(:css, "*[name='cart[allow_edit_billing_address]']")
        end
      end
    end
  end
