module MyApp
  module Test
    module ComponentObject
      class PaypalLoginAngular < CapybaraObjects::ComponentObject
        ctype "paypal_login_angular"
        locator :css, "#loginSection", wait: 30

        def login!(email:, password:)
          within_login_iframe do
        ...
        def within_login_iframe(&blk)
          Capybara.current_session.within_frame(the_iframe_name, &blk)
        rescue ::Capybara::ElementNotFound, Selenium::WebDriver::Error::StaleElementReferenceError

        end

        def the_iframe_node
          scoped_find(:css, "iframe")
        end

        def the_iframe_name
          the_iframe_node[:name]
        end

        def email_node
          find(:field, "login_email")
        end

        def password_node
          find(:field, "login_password")
        end
